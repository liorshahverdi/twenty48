...
public class Game {
	...
	public static boolean canShiftAnything(Grid x) {
		if (canBeShiftedUp(x) || canBeShiftedLeft(x) || canBeShiftedDown(x)
				|| canBeShiftedRight(x))
			return true;
		else
			return false;
		...
		while (keepPlaying) {
			...
			if (canShiftAnything(mainGrid)) {
				...
				if (nextShift.equals("W")) {
					if (canBeShiftedUp(mainGrid))
						mainGrid.shiftUp();
					else {
						System.out
								.println("Cannot be shifted up. A S D are your options");
						continue;
					}
				} else if (nextShift.equals("A")) {
					if (canBeShiftedLeft(mainGrid))
						mainGrid.shiftLeft();
					else {
						System.out
								.println("Cannot be shifted left. W S D are your options");
						continue;
					}
				} else if (nextShift.equals("S")) {
					if (canBeShiftedDown(mainGrid))
						mainGrid.shiftDown();
					else {
						System.out
								.println("Cannot be shifted down. W A D are your options");
						continue;
					}
				} else if (nextShift.equals("D")) {
					if (canBeShiftedRight(mainGrid))
						mainGrid.shiftRight();
					else {
						System.out
								.println("Cannot be shifted right. W A S are your options");
						continue;
					}
				}
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 
		 * Grid mainGrid = new Grid(); int[][] mat = mainGrid.getGrid();
		 * 
		 * mainGrid.setTile(0,0,4); mainGrid.setTile(0,1,0);
		 * mainGrid.setTile(0,2,2); mainGrid.setTile(0,3,4); //shiftRight
		 * 
		 * 
		 * mainGrid.setTile(0,0,4); mainGrid.setTile(1,0,2);
		 * mainGrid.setTile(2,0,0); mainGrid.setTile(3,0,4);
		 * 
		 * System.out.println("********************"); printMat(mat);
		 * System.out.println("********************"); mainGrid.shiftRight();
		 * printMat(mat); System.out.println("********************");
		 */
	}
}
